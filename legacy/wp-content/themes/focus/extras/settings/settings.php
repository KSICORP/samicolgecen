...
function siteorigin_settings_admin_menu() {
	$theme = wp_get_theme();
	$page = add_theme_page(
		sprintf(__( '%s Theme Settings', 'focus' ), $theme->get('Name') ),
		sprintf(__( 'Theme Settings', 'focus' ), $theme->get('Name') ),
		'edit_theme_options',
		'theme_settings_page',
...
function siteorigin_settings_render() {
	if( version_compare( get_bloginfo('version'), '3.4', '<' ) ) {
		?><div class="wrap"><div id="setting-error-settings_updated" class="updated settings-error"><p><strong><?php _e('Please update to the latest version of WordPress to use theme settings.', 'focus') ?></strong></p></div></div><?php
		return;
	}
...
function siteorigin_settings_field( $args ) {
	...
	switch ( $args['type'] ) {
		case 'checkbox' :
			...
			<label for="<?php echo esc_attr( $field_id ) ?>"><?php echo esc_attr( !empty( $args['label'] ) ? $args['label'] : __( 'Enabled', 'focus' ) ) ?></label>
			<?php
			break;
		case 'text' :
		case 'number' :
		...
		case 'media':
			if(version_compare(get_bloginfo('version'), '3.5', '<')){
				printf(__('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'focus'), admin_url('update-core.php'));
				break;
			}
			...
			$choose_title = empty($args['choose']) ? __('Choose Media', 'focus') : $args['choose'];
			$update_button = empty($args['update']) ? __('Set Media', 'focus') : $args['update'];
			...
			?>
				<div class="media-field-wrapper">
					...
					<a href="#" class="media-remove-button"><?php _e('Remove', 'focus') ?></a>
				</div>
		...
		case 'teaser' :
			...
			<a class="premium-teaser siteorigin-premium-teaser" href="<?php echo admin_url( 'themes.php?page=premium_upgrade' ) ?>" target="_blank">
				<em></em>
				<?php printf( __( 'Only available in <strong>%s</strong> - <strong class="upgrade">Upgrade Now</strong>', 'focus' ), apply_filters('siteorigin_premium_theme_name', ucfirst($theme) . ' ' . __( 'Premium', 'focus' ) ) ) ?>
		...
		case 'gallery' :
			...
			<input
				...
				type="text"
				value="<?php echo esc_attr( $current ) ?>" />
			<a href="#" class="so-settings-gallery-edit"><?php _e('Select Images', 'focus') ?></a>
			<?php
			break;
		...
		case 'pages' :
			...
			?>
			<select id="<?php echo esc_attr( $field_id ) ?>" name="<?php echo esc_attr( $field_name ) ?>">
				<option value="0"><?php _e('None', 'focus') ?></option>
		...
		case 'widget' :
			...
			if( !class_exists($args['widget_class']) ) {
				...
				echo '<div class="so-settings-widget-form"><a href="#" class="so-settings-widget-edit" data-is-setup="0" data-form="'.esc_attr($form).'">' . __('Edit', 'focus') . '</a></div>';
...
function siteorigin_settings_change_message() {
	if ( get_theme_mod( 'siteorigin_settings_changed' ) ) {
		...
		<div id="setting-updated" class="updated">
			<p>
				<strong><?php _e( 'Settings saved.', 'focus' ) ?></strong>
			</p>
		</div>
		<?php
...
function siteorigin_settings_theme_help(){
	...
	$text = sprintf(
		__( "Read %s's <a href='%s'>theme documentation</a> for help with these settings.", 'focus' ),
		ucfirst($theme_name),
...
function siteorigin_settings_media_view_strings($strings, $post){
	...
	$strings['insertIntoPost'] = __('Set Media File', 'focus');
